...
function Progress({ view }) {
  ...
  const Header = (
    <div style={{ margin: '4px 0 16px' }}>
      <div style={pLabel(fs)}>Your return to work</div>
      <h1 style={{ fontFamily: 'var(--font-display)', fontSize: fs(28), fontWeight: 500, color: 'var(--ink)', margin: '5px 0 0' }}>Fit-for-duty progress</h1>
    </div>
  );

  // --- fit-for-duty readiness signal (the thing the OT attests to) ---
  const ReadinessCard = (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 18, background: 'var(--ink-deep)', borderRadius: 22, padding: 20, color: '#fff', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ fontFamily: 'var(--font-ui)', fontSize: fs(12), fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--accent-bright)', whiteSpace: 'nowrap' }}>Fit for duty</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: fs(20), fontWeight: 500, lineHeight: 1.15, margin: '5px 0 6px' }}>Back to {PATIENT.job.toLowerCase()}</div>
  ...
  const Signals = (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 20, padding: 18, marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 14 }}>
        ...
        <span style={{ fontFamily: 'var(--font-ui)', fontSize: fs(16), fontWeight: 600, color: 'var(--ink)' }}>What we&rsquo;re tracking for the job</span>
      </div>
  ...
  // --- graded return schedule (the heart of RTW) ---
  const statusStyle = {
  ...
  const GradedReturn = (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 20, padding: 18, marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 16 }}>
        <span style={{ color: 'var(--accent)', display: 'inline-flex' }}><TIc.briefcase size={19} duotone /></span>
        <span style={{ fontFamily: 'var(--font-ui)', fontSize: fs(16), fontWeight: 600, color: 'var(--ink)' }}>Your graded return plan</span>
      </div>
      <div style={{ position: 'relative' }}>
  ...
  return (
    <div style={{ padding: '2px 20px 110px' }}>
      ...
      <div style={{ background: 'var(--gradient-recovery)', borderRadius: 20, padding: 20, marginBottom: 18, color: '#fff', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 9, fontFamily: 'var(--font-ui)', fontSize: fs(12), fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', opacity: 0.9 }}><TIc.clipboard size={16} /> Return-to-work goal</div>
        <div style={{ position: 'relative', fontFamily: 'var(--font-display)', fontSize: fs(22), fontWeight: 500, margin: '8px 0 4px' }}>{PATIENT.returnGoal}</div>
        <div style={{ position: 'relative', fontFamily: 'var(--font-ui)', fontSize: fs(14), opacity: 0.92 }}>{PATIENT.job} · target {PATIENT.targetDate}</div>
      </div>
      ...
      <SettingGroup label="Care team" fs={fs}>
        ...
        <Row fs={fs} icon="briefcase" title="Return-to-work review" sub="Therapist confirms each step" trailing={<span style={{ fontFamily: 'var(--font-ui)', fontSize: fs(13), color: 'var(--muted)', fontWeight: 600 }}>Thu</span>} />
