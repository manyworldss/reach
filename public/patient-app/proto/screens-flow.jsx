...
function Splash({ onStart }) {
  ...
  return (
    <div style={{ height: '100%', background: 'var(--gradient-recovery)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 22, position: 'relative', overflow: 'hidden', padding: 28 }}>
      ...
      <div style={{ position: 'relative', fontFamily: 'var(--font-ui)', fontSize: fs(16), color: 'rgba(255,255,255,0.9)', textAlign: 'center', maxWidth: 240, lineHeight: 1.5, animation: 'reachRise 600ms var(--ease-quiet) 680ms both' }}>Recovery, one day at a time. For the everyday movements that matter most.</div>
...
// adapts the day + feeds the picture the therapist reviews. Icons + words, never color alone.
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--surface)' }}>
      ...
      <div key={step} style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '14px 22px 0' }}>
        ...
        <p style={{ fontFamily: 'var(--font-ui)', fontSize: fs(15), color: 'var(--muted)', margin: '10px 0 4px' }}>There is no wrong answer. It just helps tune today to how you feel.</p>
...
function Today({ onStart, onCheckin, onEducation, layout, doneCount = 0, checkinDone, showClinical }) {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 13, padding: 14, background: 'var(--surface)', border: `1px solid ${work ? 'var(--apricot-ink)' : 'var(--line)'}`, borderRadius: 16, opacity: done ? 0.55 : 1 }}>
        <div style={{ width: 46, height: 46, borderRadius: 13, background: done ? 'var(--accent)' : `var(--${e.tint})`, color: done ? '#fff' : `var(--${e.tint}-ink)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none' }}>
          {done ? <FIc.check size={24} /> : work ? <FIc.hand size={22} duotone /> : <FIc.dumbbell size={21} duotone />}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            <span style={{ fontFamily: 'var(--font-display)', fontSize: fs(17), fontWeight: 500, color: 'var(--ink)' }}>{e.name}</span>
            {work && <span style={{ fontFamily: 'var(--font-ui)', fontSize: fs(9.5), fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--apricot-ink)', background: 'var(--apricot)', padding: '2px 6px', borderRadius: 6, whiteSpace: 'nowrap' }}>Daily task</span>}
          </div>
          <div style={{ fontFamily: 'var(--font-ui)', fontSize: fs(13.5), color: 'var(--muted)', marginTop: 2 }}>{e.sets} sets · {e.reps} reps · {e.everyday}</div>
        </div>
      </div>
    );
  };
  ...
  return (
    <div style={{ padding: '2px 20px 116px' }}>
        ...
        {effortFirst ? (
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: 'var(--font-ui)', fontSize: fs(14.5), marginTop: 6, opacity: 0.92 }}>Consistency is what moves your recovery. Effort counts, not scores.</div>
            </div>
          </div>
        ) : (
          <div style={{ position: 'relative' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'var(--font-ui)', fontSize: fs(12), fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--accent-bright)', whiteSpace: 'nowrap' }}><FIc.spark size={15} /> Everyday independence</div>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, margin: '10px 0 12px' }}>
              ...
              <span style={{ fontFamily: 'var(--font-ui)', fontSize: fs(15), color: 'var(--text-on-dark-muted)', paddingBottom: 4, whiteSpace: 'nowrap' }}>everyday tasks · {READINESS.band}</span>
            </div>
      ...
      {/* the tasks */}
      <div style={pLabel(fs, { marginBottom: 12 })}>Today&rsquo;s tasks · everyday movements</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: t.bg, position: 'relative' }}>
      ...
      <div key={idx} style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', animation: 'none' }}>
        ...
        <div style={{ margin: fullBleed ? '0' : '4px 18px 0', borderRadius: fullBleed ? 0 : 20, background: dark ? 'var(--surface-dark-2)' : `var(--${ex.tint})`, aspectRatio: fullBleed ? '3 / 3.4' : '4 / 3', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, color: dark ? 'var(--accent-bright)' : `var(--${ex.tint}-ink)`, position: 'relative', overflow: 'hidden', flex: 'none' }}>
          ...
          <span style={{ fontFamily: 'var(--font-ui)', fontSize: fs(14), fontWeight: 500, position: 'relative' }}>{work ? `Your therapist’s clip: ${ex.dailyTask}` : 'Watch the demonstration'}</span>
          {work && <div style={{ position: 'absolute', right: 14, top: 12, display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-ui)', fontSize: fs(11), fontWeight: 600, color: dark ? 'var(--accent-bright)' : `var(--${ex.tint}-ink)`, background: dark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.55)', padding: '4px 9px', borderRadius: 999 }}><FIc.video size={14} /> From your therapist</div>}
          {fullBleed && <div style={{ position: 'absolute', left: 16, top: 12, fontFamily: 'var(--font-ui)', fontSize: fs(12), fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: dark ? 'var(--text-on-dark-muted)' : `var(--${ex.tint}-ink)`, opacity: 0.85 }}>{work ? 'Daily task' : 'Movement'} {idx + 1} of {TODAY.length}</div>}
        </div>

        <div style={{ padding: '16px 20px 8px', display: 'flex', flexDirection: 'column' }}>
          {!fullBleed && <div style={pLabel(fs, { color: dark ? 'var(--text-on-dark-muted)' : 'var(--muted)' })}>{work ? 'Daily task' : 'Movement'} {idx + 1} of {TODAY.length} · {ex.region}</div>}
          ...
          {work && (
            <button onClick={() => setRecorded(!recorded)} style={{ display: 'flex', alignItems: 'center', gap: 13, marginTop: 14, padding: '14px 16px', width: '100%', textAlign: 'left', borderRadius: 15, border: `2px solid ${recorded ? 'var(--accent)' : (dark ? 'rgba(255,255,255,0.14)' : 'var(--border-strong)')}`, borderStyle: recorded ? 'solid' : 'dashed', background: recorded ? (dark ? 'rgba(32,196,176,0.12)' : 'var(--mint)') : 'transparent', cursor: 'pointer', WebkitTapHighlightColor: 'transparent' }}>
              ...
              <span style={{ flex: 1 }}>
                ...
                <span style={{ display: 'block', fontFamily: 'var(--font-ui)', fontSize: fs(13.5), color: t.soft, marginTop: 1 }}>{recorded ? 'Sent to your therapist for review' : 'Film it so your therapist can check your form'}</span>
              </span>
            </button>
          )}
